...
if __name__ == '__main__':
    ...
    xml_dir = 'HRSID/xml'
    xml_labels = os.listdir('HRSID/xml')
    np.random.shuffle(xml_labels)
    split_point_1 = int(len(xml_labels)/5)     #划分1/5测试
    # split_point_2 = int(len(xml_labels)/2.5)	#划分1/5验证

    # validation data
    xml_list = xml_labels[0:split_point_1]
    json_file = 'paper/instances_val2017.json'
    convert(xml_list, xml_dir, json_file)
    for xml_file in xml_list:
        img_name = xml_file[:-4] + '.jpg'
        shutil.copy(('HRSID/images/' + img_name),
                    'paper/val2017/' + img_name)
    # validation data
    # xml_list = xml_labels[split_point_1:split_point_2]
    # json_file = './instances_test2017.json'
    # convert(xml_list, xml_dir, json_file)
    # for xml_file in xml_list:
    #     img_name = xml_file[:-4] + '.jpg'
    #     shutil.copy(os.path.join(root_path, 'JPEGImages', img_name),
    #                 os.path.join(root_path, 'test2017', img_name))
    # train data
    xml_list = xml_labels[split_point_1:]
    json_file = './instances_train2017.json'
    convert(xml_list, xml_dir, json_file)
    for xml_file in xml_list:
        img_name = xml_file[:-4] + '.jpg'
        shutil.copy(('HRSID/images/' + img_name),
                    'paper/train2017/' + img_name)